...
class SearchPeople(SiteForm):
    label = 'Search for a site member'
    ...
    @form.action(label='Search', failure='handle_search_action_failure')
    def handle_search(self, action, data):
        ...
        if userId and (userId in self.siteMembers):
            self.status = 'Be joyous! You found someone.'
            userInfo = createObject('groupserver.UserFromId',
                                    self.context, userId)
            uri = userInfo.url
            return self.request.RESPONSE.redirect(uri)
        else:
            self.status = 'Could not find any site-member with the '\
                'email address <code class="email">%s</code>.' % email
